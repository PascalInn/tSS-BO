...
dim = 100
funct = botorch.test_functions.synthetic.Griewank(dim = dim)
num = 100

k = 300

fa = lambda x: funct.evaluate_true(x * 1200).detach().numpy()
...
for i in range(repeat_num):
    ...
    dataset_file = './result/dataset_%d_tSS_BO_griewank0816_%d.pkl'%(i, dim)
...
for i in range(repeat_num):
    print('start repeat, ', i)
    dataset_file = './result/dataset_%d_tSS_random_griewank_%d.pkl'%(i, dim)
    main_solver(fa, dim, bounds, init_x = None, init_y = None,
                    sigma = 0.2, mu = 0.5, c1 = None, c2 = None, allround_flag = False, greedy_flag = False,
